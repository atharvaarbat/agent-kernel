...
\textbf{H1c (starvation) and H1d (DRF) status.} Our simulation metric for starvation events counts completions where an agent exceeded $3n$ completions between its consecutive services; under the ${\approx}31\!:\!1$ skew workload, agent~0 correctly waits $\approx31$ completions between calls (due to VTime ordering), which spuriously triggers this counter. A correct starvation metric requires tracking backlogged-but-unserved agents over virtual time rather than wall completions; we leave this refinement and the full DRF/two-resource evaluation to follow-on work.
