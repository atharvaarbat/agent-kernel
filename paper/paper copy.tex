...
\paperurl{https://github.com/username/agent-kernel}
...
\begin{figure}[t]
\includegraphics[width=\textwidth]{figures/figure_1.jpg}
...
\item \textbf{An implemented runtime and evaluation harness} (\S\ref{s:implementation}, \S\ref{s:evaluation}): AgentKernel---syscall interface, budget ledger, write-ahead log, and two enforced execution forms---together with a deterministic mock-LLM harness calibrated from real traces and an adversarial suite for estimator and mediation stress.
\end{enumerate}
...
The corollary is architectural: middleware can \emph{approximate} the scheduler but cannot \emph{guarantee} it, because an agent---or a bug---can always construct the underlying client itself. This is why \S\ref{s:forms} stratifies execution forms by how strongly each closes the bypasses, and why we measure shim leakage empirically (\S\ref{s:leakage}) rather than assuming it away.
...
For existing framework code, a \textbf{compatibility shim} intercepts the LLM- and tool-client call surface and reroutes those calls as syscalls. It is a migration path, not a guarantee; by Proposition~\ref{p:mediation} it is exactly the leaky case, so we measure its leakage with an adversarial escape suite and report which invariants survive under shim-only mediation (\S\ref{s:leakage}).
...
\Procedure{dispatch}{}
    ...
    \If{$\mathit{Cand} = \varnothing$}
        \State \Return
    \EndIf
    \State $i \gets \arg\min_{i \in \mathit{Cand}} F_i$ \Comment{ties broken by agent id (I4)}
...
\subsection{Research questions and hypotheses}\label{s:questions}

\textbf{RQ0 (mediation tax).} Can complete mediation be imposed at acceptable overhead?\\
\emph{H0a:} Mediation overhead---scheduling, kernel dispatch, and WAL append---is under 5\% of wall-clock and under 1\% of token spend versus an unmediated LangGraph-style baseline on identical tasks.

\textbf{RQ1 (scheduling).} Does virtual-time fair scheduling of whole, non-preemptible LLM calls with unknown completion cost achieve bounded unfairness and starvation freedom at the runtime layer?\\
\emph{H1a:} RVT keeps Jain's fairness index \citep{Jain84} above 0.9 across 10 agents at 10:1 demand skew, versus below 0.6 for FCFS and \texttt{Promise.all}.\\
\emph{H1b:} The maximum observed service gap between continuously backlogged equal-weight agents respects Theorem~\ref{t:rvt}, and the bound is approached in the heavy-tailed adversarial case.\\
\emph{H1c:} Under mixed priority classes with aging, no agent waits more than $N$ epochs, with zero starvation events over sustained adversarial load.\\
\emph{H1d (secondary):} The DRF extension identifies each agent's dominant bottleneck and achieves per-dimension fairness comparable to H1a when agents bottleneck on different resources.

\subsection{Scheduler experiments}\label{s:experiments}

A deterministic mock-LLM load generator---latency and output-length distributions calibrated from real API traces, no spend---drives $n \in \{2, 10, 50\}$ agents under six workloads: uniform demand (baseline fairness); 10:1 demand skew (H1a); adversarial bursts, in which one agent submits back-to-back maximum-cost calls; heavy-tailed output-length distributions (the estimator-stress case, and the setting of Proposition~\ref{p:norecon}'s construction); mixed priority classes with starvation-inducing low-priority agents (H1c); and a two-resource bottleneck with one agent tokens-limited and another requests-limited (H1d).

Metrics: Jain's index \citep{Jain84} over sliding windows; maximum pairwise service gap against the Theorem~\ref{t:rvt} bound, reported as both worst case observed and full distribution; p50/p99 dispatch latency per priority class; starvation counts over 1000-epoch runs; and utilization as a work-conservation check.

Baselines: FCFS, unweighted round-robin, \texttt{Promise.all} (no scheduler), and token-bucket-per-agent (the folk remedy of \S\ref{s:alternatives}). We additionally run \textbf{estimate-only RVT}---Algorithm~\ref{alg:rvt} with the reconciliation line removed---as the ablation that instantiates Proposition~\ref{p:norecon} empirically, and \textbf{reservation on/off} to measure the fairness-versus-admission-safety trade-off identified in \S\ref{s:enforcement}.

Because a bound stated at 100K-token scale is a liveness statement rather than a practical one, we report the proven worst case \emph{and} the empirical distribution. The distribution is the operational claim; the bound is the guarantee.

\subsection{Overhead and fidelity}\label{s:overhead}

Unmediated LangGraph/AutoGen baselines run against SDK-mediated and sandboxed configurations on identical GAIA \citep{Mialon23} and $\tau$-bench \citep{Yao24} tasks at $n \in \{2, 10, 50\}$. The WASM sandbox's overhead and the kernel's dispatch-path latency are the two cost drivers and are reported separately. A kernel micro-benchmark measures syscall dispatch throughput under 50-agent concurrency, since serial dispatch is the candidate bottleneck and H0a must hold under load. Streaming overhead is measured separately per \S\ref{s:streaming}.
...
Borg \citep{Verma15}, Kubernetes, and Ray \citep{Moritz18} schedule CPU, GPU, and memory; DRF \citep{Ghodsi11} supplies the multi-resource fairness theory we adapt to (tokens/min, requests/min, dollars/epoch). Token economics for agent systems is an emerging concern \citep{TokenEcon26}. The theory transfers; the contribution here is the resource model, which is why \S\ref{s:drf} is a secondary result.
...
\textbf{Multi-resource normalization is provider-relative.} Rate limits and costs differ across providers, and tokenizers differ, so the DRF extension normalizes against per-session configured limits rather than a universal token unit. Cross-provider accounting drift is measured and reported.

\textbf{Baseline realism.} Mock-LLM fidelity is validated against real API traces before any scheduler claim is made, and all primary metrics have mechanical ground truth---no LLM judges.
...
\section{Proof of Lemma~\ref{l:identity} Alternative Derivation}\label{a:identity}
